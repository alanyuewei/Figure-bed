...
class sina_model extends CI_Model
{
     ...
     */
    public function login($u = 'a@0oo.ren', $p = "qwe123qwe")
    {
     ...
     * @return 返回的json数据
     */
    public function upload($file, $cookie, $multipart = true, $url = "http://picupload.service.weibo.com/interface/pic_upload.php?mime=image%2Fjpeg&data=base64&url=0&markpos=1&logo=&nick=0&marks=1&app=miniblog")
    {
        if ($multipart) {
    ...
    {
       $cookie = $this->login();
        $str = $this->upload($file, $cookie);
        $ret = json_decode(($str), 1);
        return  $ssl == 'https' ? $this->getImageUrl($ret['data']['pics']['pic_1']['pid']) :  $this->getImageUrl($ret['data']['pics']['pic_1']['pid'], 0, 0);
    }
